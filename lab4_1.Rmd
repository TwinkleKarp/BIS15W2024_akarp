...
```
(the 'readr::' is to be more explicit with the package)
...
Now,we are only interested in fish id and length.
```{r}
...
Load the data.  
```{r}
mammals <- readr::read_csv("mammal_lifehistories_v2.csv") #is already a data frame so don't use 'data.frame' beyond this point
```
(is already a data frame so don't use 'data.frame' beyond this point)
...
```{r}
mammals_new <- rename(mammals, genus="Genus", max_life="max. life", litter_size="litter size", afr="AFR", litters_per_year="litters/year", wean_mass="wean mass")
mammals_new
```
...
```{r}
tolower(mammals_new)
select_all(mammals_new, tolower)
```
...
```{r}
clean_names(mammals) #super handy trick for cleaning up column names
```
