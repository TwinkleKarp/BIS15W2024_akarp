...
Start by making a free account with [Stadia Maps](https://client.stadiamaps.com/signup/). You need to add a "property" which is just a name for your project. Once you have an account, you can get an API key. Use the following code to register your key.  
```{r}
...
Now we need to tell the gg map program what we want to set the bounding box to. We use the min and max values for latitude and longitude to set the range.  
```{r}
...
`ggmap` works well with `ggplot2`. To add our points we only need to specify the x and y location similar to how we made charts in previous labs. Can't use pipes for this. This will show us all the points of the spiders the data collected
```{r}
ggmap(map1) + 
