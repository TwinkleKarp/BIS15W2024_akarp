...
We can view elements in our list with double brackets. Let's view the data for Butte county.Shows one observation for Butte County
```{r}
spider_list[[3]] #use brackets because not part of tidyverse(?)
```

## Practice
1. Getting an idea of the structure of lists is a bit tricky, especially when there are many elements. Try exploring the elements in `spider_list` using names(), str(), or glimpse(). For names(), you will need to specify an element using [[]] in all of these functions.  
```{r}
...
names_list #now just has names of the counties; useful to make a vector of the names
```
...
```{r}
butte <- spider_list[["Butte"]] #storing it as an object in the data panel allows you to use it as any other object
```


## Merging Files
When you have 2 daa sets you want to combine together. We are fortunate here in that all of our data frames have the same column names. This makes it easy to merge the data together with `bind_rows()` from `dplyr`. `bind_rows()` matches columns by name.
```{r}
...
Let's assume we have a database of customers and their purchases. We want to combine them together, but they have different column names
```{r}
...
```{r}
table_B #additional customer info
```
...
```{r}
inner_exampleDF <- inner_join(table_A, table_B, by="customer_ID") #inner_join only includes the columns that both files share
inner_exampleDF #removed customer 3 and 4
```

`left_join`
Where did customer 4 go? Notice the missing data for customer 3.
```{r}
left_exampleDF <- left_join(table_A, table_B, by="customer_ID") #customer 4 is gone because wasn't include in the original left table
left_exampleDF
```
...
`full_join`
```{r}
full_exampleDF <- full_join(table_A, table_B, by="customer_ID") #doesn't care about exact matches; combines everything together
full_exampleDF
```

`anti_join`
Provides the rows in the first table for which there are not matching values in the second table.Can use as a way to troubleshoot
```{r}
...
## Practice
1. Which type of join is most appropriate in this case?  Left join is what we want because want to add the new data file(right) onto the spiders-all file(left)
```{r}
spiders_with_locs<- 
...
```{r}
#dmy(spiders_with_locs$Date) #2 NA's are because the format isn't the same across all of the dates, so makes them NA's
```
...
```{r}
mdy_hms(dateformat4) #if have info for hours, minutes, and seconds, can use
```
