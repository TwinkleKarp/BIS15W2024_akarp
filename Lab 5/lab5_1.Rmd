...
```{r}
mammals <- clean_names(mammals) #uses janitor package
```
...
Let's convert `lakeid` to a factor and have a look at which states are represented in the data. A factor is when there are only about 2 possible values that go along with a given variable
```{r}
fish$lakeid <- as.factor(fish$lakeid)
levels(fish$lakeid) #all the possible values within the variable
```
...
```{r}
filter(fish, lakeid == "AL" & length > 350) # '&' is the same as a ","
```

Notice that the `|` operator generates a different result.
```{r}
filter(fish, lakeid == "AL" | length > 350) #the '|' stands for 'or', which means it only has to satisfy one of these conditions
```
...
## this is a long and tedious way of building this type of data frame, though
