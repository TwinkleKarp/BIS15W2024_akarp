...
AI, or Artificial Intelligence, refers to computer systems that can perform tasks requiring human-like intelligence. There are two main types: Narrow AI, designed for specific tasks, and General AI, with broad cognitive abilities like humans (still theoretical). AI functionalities range from reactive machines with predefined rules to systems that can learn from data. Common applications include virtual assistants, image recognition, and recommendation algorithms.

2. What is ChatGPT?

ChatGPT is a conversational language model developed by OpenAI based on the GPT architecture. It generates human-like text responses and is trained on diverse internet text for various natural language processing tasks. Users interact by providing prompts, and the model generates contextually relevant responses.

3. What is Github Copilot?

GitHub Copilot is an AI-powered coding assistant developed by GitHub in collaboration with OpenAI. It assists developers by suggesting code snippets as they write, aiming to speed up the coding process.

4. How does coding work?

Coding involves writing instructions in a programming language to create software or applications. A programmer uses a text editor or an integrated development environment (IDE) to write code. The code is then translated into machine-readable instructions by a compiler or interpreter, allowing computers to execute the specified tasks or functions.
...
separate each directorinto their own row
```{r}
