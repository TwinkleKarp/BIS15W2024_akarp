...
3. What are the dimensions of this data frame (variables and observations)? How do you know? Please show the *dim()* that you used to determine this below.  
```{r}
dim(sleep)
```
...
```{r}
sleep$bodywt <- as.factor(sleep$bodywt)
```
...
```{r}
small_mammals <- data.frame(sleep, bodywt<= 19)
small_mammals
```

```{r}
large_mammals <- data.frame(sleep, bodywt>=200)
large_mammals
```
