...
```
still need to separate the drugs
...
tb_data
```
'key; has 2 names 'cases' and 'population', so use pivot_wider to help resolve this
...
## Practice
1. Load the `gene_exp.csv` data as a new object `gene_exp`. Are these data tidy? No, because have more than 1 variable represented in each column. Use `pivot_wider()` to tidy the data.
```{r}
...
2. Use `pivot_wider` to transform the data into wide format. Need to put the different sites into their own columns
```{r}
...
3. Now, use `pivot_longer` to transform them back to long! This is so that the data can be analyzed
```{r}
beachbugs_wide %>% 
  pivot_longer(-year,
               names_to = "site",
