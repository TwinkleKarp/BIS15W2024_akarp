...
`tabyl()` will also produce counts, but the output is different. It is just a matter of personal preference.Remember, it's a part of the 'janitor' package
```{r}
penguins %>% 
...
By using `across()` we can reduce the clutter and make things cleaner. used here for categorical variables
```{r}
penguins %>%
...
```{r}
penguins %>%
  summarize(across(contains("mm"), mean, na.rm=T)) #includes "contains("mm")" so that it works with the continuous variable
```
...
Here we summarize across all variables. the variables that arent categorical tend to have weird outputs, though.
```{r}
penguins %>%
...
penguins %>%
  group_by(species) %>% 
  summarize(across(c(contains("mm"), body_mass_g), mean, na.rm=T), #used 'c'oncatinate to include the variables(?)
            n_samples=n()) %>% 
  summarize(across(c(contains("mm"), body_mass_g), sd, na.rm=T), #cant do mean, sd, etc. in the same line
            n_samples=n())
```
