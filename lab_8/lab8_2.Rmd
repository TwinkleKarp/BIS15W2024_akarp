...
```{r}
summary(life_history) #the weaning mass mean is -427.16, which can't be possible
```
...
life_history <- read_csv("data/mammal_lifehistories_v3.csv", na= c("NA", " ", ".", "-999", "not measure")) %>% 
  clean_names() 
# you need to know how the NAs are represented in the data before doing this
#dont want to do this by default
```
...
#how can your max life span be 0?
#shows that 0 could stand for NA
```
...
#now shows that max life has a lot of missing data
#demonstrates how important it is to analyze data and find the things that dont make sense
```
...
wildlife_trade %>% 
  group_by(class) %>% 
  select(class, exporter_reported_quantity) %>% #dont have to pull out these variables- just helps to visualize
  miss_var_summary() %>% #identifying the exporters that identified the missing data for their exports
...
#very useful to identify variables with large amounts of missing data
```

```{r}
vis_miss(life_history)
#just looking at the missing data
```
