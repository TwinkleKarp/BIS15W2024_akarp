...
```{r}
my_palette <- paletteer_d("ggsci::nrc_npg") #stores your palette; 'ggsci' is the package and 'nrc_npg' is the palette
```
...
```{r}
new_palette <- paletteer_d("ButterflyColors::historis_acheronta")
```
...
```
```{r}
q+scale_colour_manual(values=new_palette)
#?
```
