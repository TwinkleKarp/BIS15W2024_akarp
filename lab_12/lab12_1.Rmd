...
#install.packages("ggthemes") #don't knit and install packages
```
...
Let's play with the colors. This shows all 657 of R's built-in colors. Notice that `color` and `fill` do different things! These colors should be used with consistency especially when building reports of data; reuse them
```{r}
#grDevices::colors()
```
...
Using the table from above, make categories
```{r}
homerange %>% 
