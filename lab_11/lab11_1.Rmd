...
This is not the best plot because can't really see the means of the smaller orders. The y-axis is not on a log scale. We can fix this by adding `scale_y_log10()`. can also do this for the x-axis 'scale_x_log10()`
```{r}
life_history %>% 
  group_by(order) %>% 
  summarise(mean_mass=mean(mass, na.rm = T)) %>% 
...
life_history %>% 
  ...
  scale_y_log10()
```
there is one massive outlier affecting the rest of the data. Could remove it but need to say
...
Make a plot that compares age and height of elephants. (a scatter plot)
```{r}
elephants %>% 
  ggplot(aes(x=age, y=height))+
  geom_point()+
  geom_smooth(method=lm, se=F) #se=F means we're not showing standard error- aesthetic choice
```
...
```{r}
life_history %>% 
  ggplot(aes(x=gestation, y=log10(mass), size=mass))+ # t=log10() is another way to log scale y
  geom_point(na.rm=T)
```
