...
A shiny app consists of 3 parts: a user interface (ui) function, server function, and run function.
...
```{r, eval=F, echo=T}
ui <- fluidPage(
    selectInput("x", "Select X Variable", choices = c("bill_length_mm", "bill_depth_mm", "flipper_length_mm", "body_mass_g"), selected = "bill_length_mm") #all the visual things, 'x' is the name of the
)
...
want to make sure you build your plot at least once before making an app to make sure it works right
```{r}
penguins %>% 
...
ui <- fluidPage(
    ...
    plotOutput("plot", width="500px", height="400px") #providing the size/aspect of the visual we're producing
)
