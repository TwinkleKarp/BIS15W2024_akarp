...
ui <- fluidPage(titlePanel("Log 10 Homerange by Taxon"), #adds a title to the app
                selectInput("x", "Select Taxon", #'select input' refers to drop down menu
...
server <- function(input, output, session) {
  output$plot <- renderPlot({
    homerange %>% 
      ...
      ggplot(aes(x=log10.hra))+ #not aes_string because just assigned reactive part to the filter- not this part; dont have to assign reactivity just to x and y
...
server <- function(input, output, session) { }
        ...
        session$sessionEnded(stopApp) #automatically stop the app when we close it
...
ui <- fluidPage(    
  ...
  #shinythemes::themeSelector(),  # <--- Add this somewhere in the UI to help see what themes there are. after choosing one comment it out and use the line below
