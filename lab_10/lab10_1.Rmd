...
ggplot(data=iris, #specify the data
  ...
  geom_point() #refer to the data vizualization pdf in lab 10 folder to determine what command to use for a plot
```
...
In the case below, we are exploring whether or not there is a relationship between animal mass and home range. We are using the **log transformed values** because there is a large difference; too extreme difference in mass and home range among the different species in the data.
```{r}
names(homerange)
```
...
```{r}
ggplot(data=homerange, mapping=aes(x=log10.mass, y=log10.hra))+ # the + indicates we're adding a layer to the plot
  geom_jitter()
```
...
homerange %>% 
  ...
  geom_bar() #good for counts/categorical data, but wont work for continuous data
```
...
homerange %>% 
  ...
  ggplot(aes(y=common.name, x=log10.mass))+ #notice the switch in x and y on the graph by putting y first
  geom_col()
...
homerange %>% 
  ...
  geom_col()#this portion shows the graph
```
can also use geom_bar for this (?)
```{r}
homerange %>% 
  filter(taxon== "mammals") %>% 
  count(trophic.guild) %>% #this portion shows just the raw numbers and producing the x & y- good to include this portion in addition to the graph
