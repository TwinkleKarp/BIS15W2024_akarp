...
```{r}
penguins %>% 
  count(island) #island is x axis and 'n' is y axis
```
...
```{r}
penguins %>%
  filter(sex!="NA") %>% #pulls out NA sex
  group_by(sex) %>% 
...
```
This boxplot shows the exact same data, but is easier to do and read.
```{r}
homerange %>% 
...
homerange %>% 
  ...
  geom_boxplot()+
  coord_flip() #to flip axes and make the data more clear
```
